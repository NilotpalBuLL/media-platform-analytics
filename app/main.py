...
# -------------------- App --------------------
app = FastAPI(title="Media Platform API (Task2 Analytics)", version="0.2.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
class MediaViewLog(Base):
    ...
    viewed_by_ip = Column(String, index=True, nullable=False)
...
# -------------------- Task 2: Analytics routes --------------------
@app.post("/media/{media_id}/view", status_code=201)
def log_view(media_id: int, request: Request, db: Session = Depends(get_db), current: AdminUser = Depends(get_current_user)):
    media = db.get(MediaAsset, media_id)
    if not media:
        raise HTTPException(status_code=404, detail="Media not found")

    # Get client IP (support X-Forwarded-For if behind proxy)
    ...
    # Create log entry
    log = MediaViewLog(media_id=media.id, viewed_by_ip=client_ip)
    db.add(log)
    db.commit()
    db.refresh(log)

    return {"status": "logged", "media_id": media.id, "view_id": log.id, "ip": client_ip, "timestamp": log.timestamp.isoformat()}


@app.get("/media/{media_id}/analytics", response_model=AnalyticsOut)
def media_analytics(media_id: int, db: Session = Depends(get_db), current: AdminUser = Depends(get_current_user)):
    media = db.get(MediaAsset, media_id)
    if not media:
        raise HTTPException(status_code=404, detail="Media not found")

    # Query all view logs for this media
    logs = db.query(MediaViewLog).filter(MediaViewLog.media_id == media_id).all()

    total_views = len(logs)
    unique_ips = len(set(l.viewed_by_ip for l in logs))

    views_per_day = {}
    for l in logs:
        day = (l.timestamp.date()).isoformat()
        views_per_day[day] = views_per_day.get(day, 0) + 1

    return {"total_views": total_views, "unique_ips": unique_ips, "views_per_day": views_per_day}


# -------------------- Bonus: validate token & log view (public stream consumer) --------------------
...
def stream(token: str, request: Request, db: Session = Depends(get_db)):
    ...
    # Log the view (this one is from the public stream access; IP is recorded)
